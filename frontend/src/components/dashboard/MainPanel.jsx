...
const MainPanel = ({ currentSymbol, setCurrentSymbol, toggleSidebar, toggleChat }) => {
    ...
    const fetchData = async () => {
      try {
        const [quoteRes, histRes, newsRes, watchlistRes] = await Promise.all([
          axios.get(`http://localhost:5001/api/stocks/quote?symbol=${currentSymbol}`),
          axios.get(`http://localhost:5001/api/stocks/history?symbol=${currentSymbol}&range=${timeframe}`),
          axios.get(`http://localhost:5001/api/stocks/news?symbol=${currentSymbol}`),
          axios.get(`http://localhost:5001/api/watchlist`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }})
        ]);
  ...
  const toggleWatchlist = async () => {
    try {
      ...
      if (isInWatchlist) {
        await axios.delete(`http://localhost:5001/api/watchlist/${currentSymbol}`, { headers: { Authorization: `Bearer ${token}` }});
        setIsInWatchlist(false);
      } else {
        await axios.post(`http://localhost:5001/api/watchlist`, { symbol: currentSymbol }, { headers: { Authorization: `Bearer ${token}` }});
  ...
  useEffect(() => {
    ...
    const search = async () => {
      try {
        const res = await axios.get(`http://localhost:5001/api/stocks/search?q=${searchQuery}`);
